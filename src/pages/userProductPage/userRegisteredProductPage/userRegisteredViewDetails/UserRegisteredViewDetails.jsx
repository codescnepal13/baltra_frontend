...
const UserRegisteredViewDetails = () => {
  ...
  // ── TODO: confirm real field name with backend. This should be the
  // admin-panel product photo, matched by model_num — NOT the photo
  // the customer uploaded during registration. Falls back to the
  // customer image only if the admin-matched one isn't returned yet. ──
  const displayProductImage =
    singleAddedProduct?.admin_product_image ||
    singleAddedProduct?.product_image;

  // ── Only customer-uploaded supporting documents. Damaged product
  // image/video are complaint/service evidence and belong on the
  // Service & Tickets page, not here. ──
  const mediaItems = [
    {
      type: "image",
      src: singleAddedProduct?.bill_image_one,
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50">
      <div className="mx-auto max-w-5xl px-4 py-6 sm:py-8 lg:px-8">
        ...
        {/* ── Bottom Grid: Product Details + Store + Documents ── */}
        <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
          {/* Product Details Card */}
          <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
            <p className="mb-4 text-[11px] font-semibold uppercase tracking-widest text-gray-400 font-gothamNarrow">
          ...
          {/* Documents Card — customer-uploaded bill & warranty images only */}
          {mediaItems.length > 0 && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5 md:col-span-2">
              <p className="mb-4 text-[11px] font-semibold uppercase tracking-widest text-gray-400 font-gothamNarrow">
                Documents & images
