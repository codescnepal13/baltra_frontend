...
const Usage = ({ addProduct }) => {
  const imageToDisplay = addProduct?.usage_images?.slice(0, 2) || [];
  ...
  const usageText = addProduct?.usage?.replace(/<ul>.*<\/ul>/s, "").trim();

  const listItems = extractListFromUsage(addProduct?.usage);
  return (
    <>
      <div className="w-full bg-white py-2">
        ...
        {addProduct?.usage_images[2]?.image_url && (
          <div className="flex justify-center mt-6">
            <img
              className="w-full md:w-96 h-auto object-contain"
              src={addProduct?.usage_images[2]?.image_url}
